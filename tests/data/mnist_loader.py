    ...
    def __init__(
        ...
        try:
            self.dataset = torchvision.datasets.MNIST(
                root=str(data_dir), train=train, download=True, transform=self.transform
            )
            logger.info(
                f"MNIST数据集加载成功: {'训练集' if train else '测试集'}, 样本数: {len(self.dataset)}"
            )
...
def create_mnist_loaders(
    ...
    full_train_dataset = MNISTFlowDataset(
        train=True, target_size=target_size, data_dir=data_dir
    )
    ...
    val_indices = torch.randperm(total_size)[
...
def create_test_loader(
    ...
    test_dataset = MNISTFlowDataset(
        train=False, target_size=target_size, data_dir=data_dir
    )
...
class MNISTTestSampler:
    ...
    def sample_pair(self, batch_size: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        x_0 = generate_noise_batch(
            batch_size, shape, self.device, noise_type="gaussian"  # type: ignore
        )

        return x_0, x_1

    def sample_single(
        self, batch_size: int, distribution: str = "target"
    ) -> torch.Tensor:
        """采样单一分布的批次.
