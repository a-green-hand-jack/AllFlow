...
class OptimalTransportBase(ABC):
    ...
    def _solve_approx_ot(
        ...
        for i in range(batch_size):
            ...
            transport_plan[i, j] = torch.tensor(1.0 / batch_size, dtype=transport_plan.dtype)
            used_targets.add(j)
...
class SO3OptimalTransport(OptimalTransportBase):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if distance_metric not in valid_metrics:
            raise ValueError(f"不支持的距离度量: {distance_metric}，支持: {valid_metrics}")
    ...
    def compute_distance_matrix(
        ...
        if x_0.shape[-1] != 4 or x_1.shape[-1] != 4:
            raise ValueError(f"SO3输入必须是四元数，期望最后一维为4，得到: {x_0.shape}, {x_1.shape}")
        ...
        if self.distance_metric == "geodesic":
            return self._compute_geodesic_distance_matrix(x_0_normalized, x_1_normalized)
            ...
            return self._compute_frobenius_distance_matrix(x_0_normalized, x_1_normalized)
        else:
    ...
    def _compute_chordal_distance_matrix(
        ...
        # 计算两种可能的距离
        dist_pos = torch.norm(q_0_expanded - q_1_expanded, dim=2)  # (batch_size, batch_size)
        dist_neg = torch.norm(q_0_expanded + q_1_expanded, dim=2)  # (batch_size, batch_size)
    ...
    def _compute_frobenius_distance_matrix(
        ...
        # 计算所有配对的距离矩阵
        distances = torch.zeros(batch_size, batch_size, device=R_0.device, dtype=R_0.dtype)
    ...
    def _quaternion_to_rotation_matrix(self, quaternions: torch.Tensor) -> torch.Tensor:
        ...
        R = torch.zeros(batch_size, 3, 3, device=quaternions.device, dtype=quaternions.dtype)
